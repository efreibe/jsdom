...
const proxyTarget = {
  getWithProxy(target, P, receiver) {
    const value = Reflect.get(target, P, receiver)
    return (P === utils.wrapperSymbol && !value.__proxyfied) ?
      new Proxy(value, {
        get: function (target, name) {
          if (name === '__proxyfied') return true;
          const value = target[name];
          return value ? value : target.elements[name];
        }
      }) : value;
  },
  ...
  get(target, P, receiver) {
    if (typeof P === "symbol") {
      return this.getWithProxy(Reflect.get(target, P, receiver));
    }
...
class HTMLFormElementImpl extends HTMLElementImpl {
  constructor(...args) {
    ...
    return new Proxy(this, proxyTarget);
  }
